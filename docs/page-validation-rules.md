...
| **SS-003** | error, or warning on a legitimately-empty input | every declared input | An input file missing, unparseable, holding the wrong key, holding a non-array, or empty; a corpus file missing or zero-length; a model field empty on **every** page; zero pages or zero indexable pages | The 2026-08-21 sweep found the same defect in seventeen places: a rule producing no output because its input was unusable, which is byte-identical to a rule producing no output because the site is fine. Rather than patching seven rules, the honesty lives in one place and the next rule added inherits it. Logic is a pure function in `scripts/lib/input-guard.mjs` so it is tested by handing it broken inputs, not by breaking real files. **Findings are never suppressible** — a suppressed input guard is the bug it exists to catch. **It catches TOTAL regressions, not partial ones:** a field empty on every page fires, a field empty on half the pages does not, and nothing else would catch that either. A per-page assertion would false-positive on pages that legitimately carry no JSON-LD or no outbound links, and a rule that cries wolf gets ignored — the mechanism that produced every finding in the sweep. **A green SS-003 is therefore not evidence that the extractor is healthy for every page.** |
...
| **SS-104** | error | all 32 | `inSitemap && noindex` | **Phase 1 finding #2.** `/privacy-policy` was in the sitemap and served `noindex, follow` at the same time, with 26 inbound internal links — every footer on the site. The sitemap asked for indexing and the page refused, and nothing anywhere compared the two. Fixed in `6b67c16`; this rule is what stops it recurring. |
| **SS-105** | error | all 32 | An `.html` file in the publish directory absent from `ALLOW_HTML` | Standing guard on `c123924`. Before that commit a stray, orphaned or draft `.html` left in the publish directory was public, and no amount of reading the config would show it. |
...
| ID | Severity | Scope | Catches | Why it exists |
|---|---|---|---|---|
| **SS-201** | error, or a **route-skip** | indexable | `sitemapLastmod` ≠ the date of the newest commit touching `sourceFile`, **excluding** SHAs in `data/freshness-exclusions.json` | **Phase 1 finding: 24 of 26 routes drifted.** Raw git dates are unusable on their own — commit `acbeda6` added a `<main>` landmark to all 32 files in one pass and reset every date to 2026-08-20, so a rule keyed to git alone reports 26 content changes that did not happen, which trains you to ignore it. The exclusion list is what makes the signal real. A SHA belongs there only if it changed no published copy at all. |
...
| **SS-605** | error | routes in `data/stated-correction-counts.json` | A declared route with no `data-correction-count` attribute; an undeclared page that carries one; or a stated count disagreeing with the distinct slugs marked | `/working-with-claude-blog` enumerates its corrections in prose. That sentence went stale the moment a fifth was added — it said four — and grepping for the new heading would have said everything was fine. Same class as the slide count in eleven places. **The expectation lives outside the page** (`data/stated-correction-counts.json`) — B7 in `docs/validator-blind-spots.md`: the rule used to be gated on `data-correction-count`, so deleting the attribute deleted the check and left the prose claim unverified. Declaring the route separately means the page cannot silence it, the same shape as `og-overrides.json`. Counts **distinct slugs, not elements**: one correction can be surfaced twice on a page (`cross-session-memory` has a section and an FAQ answer), and a reader counts corrections, not mentions. |
