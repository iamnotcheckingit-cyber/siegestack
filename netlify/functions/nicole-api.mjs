/**
 * The private thread at /nicole. Two people, one channel, nothing ever deleted.
 *
 * WHAT THIS IS
 * ------------
 ...
 *   NICOLES_PIN            required. Opens this thread and no other.
 *   NICOLE_SESSION_SECRET   optional. Signs session cookies. If unset, it is
 *                          derived from the two PINs -- which means changing a
 *                          PIN signs everyone out, and that is the correct
 *                          behaviour anyway.
 *   NICOLE_MSG_KEY          optional. 32 bytes, hex or base64, for message
 *                          encryption. If unset it is derived from the two
 *                          PINs.
 *
 *                          *** READ THIS BEFORE CHANGING A PIN ***
 *                          With NICOLE_MSG_KEY unset, the encryption key depends
 *                          on the PINs, so changing a PIN makes every existing
 *                          message permanently unreadable. Set NICOLE_MSG_KEY
...
const PEOPLE = {
  ...
  nicole: { id: 'nicole', name: 'Nicole', pinVar: 'NICOLES_PIN' },
};
...
function derive(purpose) {
  return crypto
    .createHash('sha256')
    .update(`siegestack/nicole/${purpose}|${process.env.DADS_PIN || ''}|${process.env.NICOLES_PIN || ''}`)
    .digest();
}

function sessionKey() {
  const explicit = process.env.NICOLE_SESSION_SECRET;
  return explicit ? crypto.createHash('sha256').update(explicit).digest() : derive('session');
}

function messageKey() {
  const raw = (process.env.NICOLE_MSG_KEY || '').trim();
  ...
  if (buf.length !== 32) {
    console.error('NICOLE_MSG_KEY is set but is not 32 bytes; falling back to the derived key');
...
const guardStore = () => getStore({ name: 'nicole-guard', consistency: 'strong' });
...
function guardKey(ip) {
  return `fail/${crypto.createHash('sha256').update(`nicole|${ip}`).digest('hex').slice(0, 32)}`;
}
...
const threadStore = () => getStore({ name: 'nicole-thread', consistency: 'strong' });
...
const FILE_DIR_CANDIDATES = [
  () => path.resolve(process.cwd(), 'private-files-nicole'),
  () => path.resolve(process.cwd(), '../private-files-nicole'),
  () => new URL('../../private-files-nicole/', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1'),
];
...
function fileDir() {
  ...
  console.error(JSON.stringify({ event: 'NICOLE_FILE_DIR_MISSING', cwd: process.cwd() }));
  cachedDir = null;
  return null;
}
 ...
 */
const filesStore = () => getStore({ name: 'nicole-files', consistency: 'strong' });
  ...
  try {
    ...
    const found = await Promise.all(
      blobs.map(async (b) => {
        ...
        return {
          name: stored,             // what /api/nicole/file wants
    ...
    console.error(JSON.stringify({ event: 'NICOLE_UPLOAD_LIST_FAILED', detail: String(err?.message || err).slice(0, 200) }));
    return [];
  }
}
...
const presenceStore = () => getStore({ name: 'nicole-presence', consistency: 'strong' });
...
const otherPerson = (who) => (who === 'dad' ? 'nicole' : 'dad');
 ...
 */
const keyStore = () => getStore({ name: 'nicole-keys', consistency: 'strong' });
...
async function vapid() {
  ...
  if (!keys?.publicKey || !keys?.privateKey) {
    ...
    console.log(JSON.stringify({ event: 'NICOLE_VAPID_GENERATED' }));
  }
...
const pushStore = () => getStore({ name: 'nicole-push', consistency: 'strong' });
 ...
 */
const callStore = () => getStore({ name: 'nicole-call', consistency: 'strong' });
    ...
    await Promise.all(blobs.map(async ({ key }) => {
      ...
      try {
        ...
        if (err?.statusCode === 404 || err?.statusCode === 410) {
          ...
          console.error(JSON.stringify({ event: 'NICOLE_PUSH_FAILED', status: err?.statusCode || 0 }));
        }
      }
    }));
  } catch (err) {
    // A failed notification must never fail the message that triggered it.
    console.error(JSON.stringify({ event: 'NICOLE_PUSH_ERROR', detail: String(err?.message || err).slice(0, 200) }));
  }
}
 ...
 * code read -- a PIN under the wrong name, then GIPHY_KEY for this. Reading both
  ...
  try {
    ...
    if (action === 'export') {
      ...
      return new Response(JSON.stringify({ exportedAt: new Date().toISOString(), count: all.length, messages: all.filter(Boolean) }, null, 2), {
        headers: {
          'Content-Type': 'application/json',
          'Content-Disposition': `attachment; filename="nicole-thread-${new Date().toISOString().slice(0, 10)}.json"`,
    ...
    console.error(JSON.stringify({ event: 'NICOLE_API_ERROR', action, detail: String(err?.message || err).slice(0, 300) }));
